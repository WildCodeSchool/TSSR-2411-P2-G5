...
create_info_display() {
    ...
    case "$type" in
        "header")
            ...
            id "$user" >> "$filename"
            resultat=$(executer_commande "getent passwd "$user" | cut -d: -f5 >> "$filename"")
            ;;
        "commands")
        ...
        "logins")
            ...
            resultat=$(executer_commande "last -a | grep "$user" | head -n 20 >> "$filename"")
            ;;
        "processes")
            echo "--- Processus en cours de $user ---" > "$filename"
            resultat=$(executer_commande "ps aux | grep "^$user" >> "$filename"")
            ;;
        "network")
            echo "--- Activité réseau de $user ---" > "$filename"
            resultat=$(executer_commande "netstat -tuln | grep "$user" >> "$filename"")   
            ;;
        "files")
            echo "--- Fichiers récemment modifiés par $user ---" > "$filename"
            resultat=$(executer_commande "find "/home/$user" -type f -mtime -7 | head -n 50 >> "$filename"")
            
            echo -e "\n=== Fichiers récemment créés par $user ===" >> "$filename"
            resultat=$(executer_commande "find "/home/$user" -type f -ctime -7 | sort -r | head -n 50 >> "$filename"")
            ;;
        *)
