function Test-Prerequisites {
    # Vérifier si Posh-SSH est installé
    if (!(Get-Module -ListAvailable -Name Posh-SSH)) {
        $install = [System.Windows.Forms.MessageBox]::Show(
            "Le module Posh-SSH est requis mais n'est pas installé. Voulez-vous l'installer maintenant?",
            "Module manquant",
...
function Stop-Script {
    if ($global:sshSession) {
        try {
            $global:sshSession | Remove-SSHSession
        }
        catch {
            Write-Log "Erreur lors de la fermeture de la session SSH: $_" -Level Warning
        }
    }
    Write-Log "Arrêt du script" -Level Information
    exit
}
...
function Start-Script {
    ...
    if (!(Test-Prerequisites)) {
        [System.Windows.Forms.MessageBox]::Show("Prérequis manquants. Le script ne peut pas continuer.", "Erreur")
        return
    }
    ...
    Show-MainMenu
        ...
        Write-Log "Erreur lors du démarrage du script: $_" -Level Error
        [System.Windows.Forms.MessageBox]::Show("Erreur lors du démarrage : $_", "Erreur")
    }